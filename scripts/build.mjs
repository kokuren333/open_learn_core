...
import { renderConcept, renderGraph, renderIndex } from "../src/web/render.mjs";
...
for (const concept of concepts) await writeFile(path.join(site, "concepts", `${concept.id}.html`), renderConcept({ concept, conceptsById, sourceById }), "utf8");
...
console.log(`Built ${concepts.length + 2} HTML pages in ${path.relative(root, site)}/`);
