...
for (const domain of domains) {
  ...
  const coreContentById = new Map();
  try {
    const coreContent = JSON.parse(await readFile(path.join(domain.root, "data", "core-content", "core-content.json"), "utf8"));
    for (const content of coreContent.concepts ?? []) coreContentById.set(content.core_concept, content);
  ...
  for (const concept of coreConcepts) await writeFile(path.join(output, "concepts", `${concept.id}.html`), renderCoreConcept({ concept, conceptsById: coreConceptsById, content: coreContentById.get(concept.id), experience: learningExperienceByConcept.get(concept.id) }), "utf8");
