...
export async function loadDataset(root = process.cwd(), { dataRoot = path.join(root, "data"), schemasRoot = path.join(root, "schemas") } = {}) {
  const [concepts, curricula, evidenceItems, evidenceReviews, curriculumDecisions, visuals, sources] = await Promise.all([
    readJsonDirectory(path.join(dataRoot, "concepts")),
    readJsonDirectory(path.join(dataRoot, "curricula")),
    readJsonDirectory(path.join(dataRoot, "evidence", "items")),
    readJsonDirectory(path.join(dataRoot, "evidence", "reviews")),
    readJsonDirectory(path.join(dataRoot, "curriculum-decisions")),
    readJsonDirectory(path.join(dataRoot, "visuals")),
    readJson(path.join(dataRoot, "sources", "sources.json"))
  ]);
  return {
    root,
    dataRoot,
...
async function loadSchemas(schemaRoot) {
  const names = ["concept", "curriculum", "source", "evidence-item", "evidence-review", "curriculum-decision", "visual-artifact", "build-report"];
  const entries = await Promise.all(names.map(async (name) => [name, await readJson(path.join(schemaRoot, `${name}.schema.json`))]));
...
export async function validateDataset(dataset, { schemas = null } = {}) {
  const loadedSchemas = schemas ?? await loadSchemas(dataset.schemasRoot ?? path.join(dataset.root, "schemas"));
...
export async function validateConcept(concept, root = process.cwd()) {
  const schemaRoot = existsSync(path.join(root, "schemas")) ? path.join(root, "schemas") : root;
