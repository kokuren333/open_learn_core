...
export function auditCourse({ course, modules = [], units = [], videoUnitIds = [], moduleExerciseSets = [], cumulativeReviews = [], notationAvailable = true }) {
  ...
  if (moduleExerciseSets.length < 1) issues.push(issue("module-level exercise architecture has no exercise set", "Add at least one Module Review set linked to a Module."));
  if (cumulativeReviews.length < 1) issues.push(issue("cumulative review architecture is missing", "Add a cross-Module cumulative review definition."));
