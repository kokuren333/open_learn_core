...
function renderLearnerBlock(block, lesson, index) {
  ...
  const answer = block.answer ? `<details class="learner-answer"><summary>答えと解説を見る</summary><p>${text(block.answer)}</p></details>` : "";
  const prompt = block.question ?? lesson?.learner_prompt;
  const promptHtml = prompt ? `<details class="learner-checkpoint"><summary>少し考えてみる</summary><p>${text(prompt)}</p>${block.hint ? `<details class="learner-hint"><summary>ヒント</summary><p>${text(block.hint)}</p></details>` : ""}${answer}</details>` : "";
  const worked = block.worked_example;
  const workedHtml = worked ? `<details class="learner-reveal worked-detail"><summary>解法と理由を見る</summary><p><strong>問題：</strong>${text(worked.problem)}</p><ol>${worked.reasoning_steps.map((step) => `<li>${text(step)}</li>`).join("")}</ol><p><strong>計算：</strong>${text(worked.calculations)}</p><p><strong>解釈：</strong>${text(worked.interpretation)}</p><p><strong>一般化：</strong>${text(worked.generalizable_takeaway)}</p></details>` : "";
  const challenge = block.misconception_challenge;
  const challengeHtml = challenge ? `<details class="learner-reveal misconception-detail"><summary>よくある勘違いを見る</summary><p><strong>ありがちな道筋：</strong>${text(challenge.expected_wrong_path)}</p><p><strong>修正：</strong>${text(challenge.correction)}</p><p>${text(challenge.explanation)}</p><p><strong>別の例で確認：</strong>${text(challenge.transfer_check)}</p></details>` : "";
  const equations = lesson?.equations?.length ? `<div class="learner-equations"><span>式で確認</span>${list(lesson.equations)}</div>` : "";
  const transition = block.representation ? `<p class="learner-transition">視点を切り替える：${text(block.representation)}</p>` : "";
  return `<article class="learner-block" id="learner-block-${index + 1}"><div class="learner-body">${text(lesson?.body ?? lesson?.explanation ?? "")}</div>${transition}${equations}${promptHtml}${workedHtml}${challengeHtml}${lesson?.takeaway ? `<p class="learner-takeaway">${text(lesson.takeaway)}</p>` : ""}</article>`;
}
